...
- **Round codes (CONFIRMADO em 2026-06-26):** a TheSportsDB usa a convenção "times restantes" para o knockout da Copa 2026, via `eventsround` (endpoint **sem teto** no free tier): **Round of 32 = `r=32`** (confirmado — 4 jogos já publicados com `idTeam` reais que batem com `id_externo`). R16/Quartas/Semi/Final muito provavelmente `16`/`8`/`4`/`2` (ainda vazios; preenchem conforme as fases avançam). A API publica os confrontos **incrementalmente** (4 de 16 no R32 hoje). NOTA: `r=2` colide com a rodada 2 de grupos — irrelevante, pois o casamento é por par de times (o par do Final nunca coincide com um par de grupos).
- **Decisão:** o sync de knockout usa `eventsDasRodadas(config('thesportsdb.rodadas_mata_mata'))` com `rodadas_mata_mata = [32, 16, 8, 4, 2]` (ajustável). Casa por **par de times**.
- **Bolão atual:** participantes derivados (`ServicoResultadosTorneio`) são persistidos na linha do jogo; o evento `r=32`+ casa por par de times e traz o resultado.
- **2º bolão (mata-mata puro):** **preenchido direto da API, rodada a rodada** — cada código de rodada (`r=32`, `r=16`, …) preenche os jogos-placeholder daquela fase com os times reais (mapeando `idTeam`→`id_externo`), ordenando por data. Cada fase é espelhada da API independentemente (não deriva vencedor→próxima fase). Rede de segurança manual do admin permanece.
